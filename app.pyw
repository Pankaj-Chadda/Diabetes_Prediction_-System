...
def show_entry_fields():
    ...
    predict_label=Label(master)
    if result == 0:
        predict_label.forget()
        predict_label=Label(master, text="Non-Diabetic").grid(row=31)
    else:
        predict_label.forget()
        predict_label=Label(master, text="Diabetic").grid(row=31)
